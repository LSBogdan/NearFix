...
const EditCommentModal = ({ isOpen, onClose, onCommentUpdated, comment }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!hasChanges()) {
      setError('No changes were made to the comment.');
      return;
    }
    ...
    try {
      const response = await api.put(ENDPOINTS.POSTS.COMMENT_DETAIL(comment.postId, comment.commentId), {
        description,
        isEdited: true
      });
      ...
      <div className="absolute inset-0 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl p-8 w-full max-w-2xl mx-4 shadow-xl transform transition-all duration-300 hover:shadow-2xl">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-[#708eb3]">Edit Comment</h2>
            <button
              onClick={onClose}
