...
const UpdateGarageModal = ({ isOpen, onClose, garage, onGarageUpdated }) => {
  ...
  const [previewUrl, setPreviewUrl] = useState(garage?.photoUrl || null);
  ...
  useEffect(() => {
    if (garage) {
      ...
      setPreviewUrl(garage.photoUrl || null);
      setPhoto(null);
      setDocument(null);
    }
  }, [garage]);
  ...
  const hasChanges = () => {
    ...
    if (formData.name !== garage.name) return true;
    
    // Check if address has changed
    const currentAddress = garage.address || {};
    if (formData.address.street !== currentAddress.street ||
        formData.address.number !== currentAddress.number ||
        formData.address.city !== currentAddress.city ||
        formData.address.country !== currentAddress.country ||
        formData.address.zipCode !== currentAddress.zipCode) {
      return true;
    }
    
    // Check if schedule has changed
    const currentSchedule = garage.schedule || [];
    if (formData.schedule.length !== currentSchedule.length) return true;
    
    for (let i = 0; i < formData.schedule.length; i++) {
      const currentDay = currentSchedule[i] || {};
      const newDay = formData.schedule[i];
      if (newDay.isClosed !== currentDay.isClosed ||
          newDay.openingTime !== currentDay.openingTime ||
          newDay.closingTime !== currentDay.closingTime) {
        return true;
      }
    }
    
    // Check if photo or document has been added
    if (photo || document) return true;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.put(
        ENDPOINTS.GARAGES.DETAIL(garage.garageId),
        formDataToSend,
        {
          headers: {
      ...
      setError(err.response?.data?.message || 'Failed to update garage. Please try again.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="bg-white rounded-2xl w-full max-w-lg mx-4 shadow-xl transform transition-all duration-300 hover:shadow-2xl max-h-[90vh] overflow-y-auto">
          <div className="p-5">
            <div className="flex justify-between items-center mb-3">
              <h2 className="text-2xl font-bold text-[#708eb3]">Update Garage</h2>
              <button
                onClick={handleClose}
                    ...
                    {formData.schedule.map((day, index) => (
                      ...
                      >
                        <div className="w-28 flex-shrink-0">
                          <span className={`text-sm font-medium ${day.isClosed ? 'text-gray-400' : 'text-[#4a5568]'}`}>{getDayName(day.dayOfWeek)}</span>
                        </div>
                ...
                <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <div className="flex flex-col items-center space-y-2">
                      <div className="relative group">
                        ...
                        >
                          <div className={`h-full w-full flex items-center justify-center ${previewUrl ? 'transition-transform duration-300 group-hover:scale-110' : ''}`}>{previewUrl ? (
                            <img
                              src={previewUrl}
                              alt="Garage preview"
                  ...
                  <div>
                    <div className="flex flex-col items-center space-y-2">
                      <div className="relative group">
                        <div
                          className={`h-20 w-20 rounded-lg overflow-hidden transition-all duration-300 ${document || garage?.documentUrl ? 'ring-2 ring-[#92a8bf] shadow-md' : 'bg-[#92a8bf]'} cursor-pointer`}
                            ...
                            {document ? (
                              ...
                              <div className="h-full w-full bg-white flex flex-col items-center justify-center p-2 transform transition-transform duration-300 group-hover:scale-110">
                                ...
                                <a
                                  ...
                                  className="text-[8px] text-[#708eb3] font-medium text-center truncate w-full underline hover:text-[#92a8bf]"
                                >
                                  {garage.documentUrl.split('/').pop() || 'View Document'}
                                </a>
                              </div>
                            ) : (
                              <div className="h-full w-full flex items-center justify-center">
                      ...
                      <div className="text-center">
                        <p className="text-[10px] text-[#819bb9] font-medium">
                          {document ? (
                            ...
                            <span className="flex items-center justify-center space-x-1">
                              <span>Existing document! Click to change</span>
                            </span>
                          ) : (
                            <span className="flex items-center justify-center space-x-1">
...
export default UpdateGarageModal;
